...
- config.py: `STATE_DIR` (~/.auto-torrent/downloads/), `STREAM_DIR` (~/.auto-torrent/stream), `CACHE_DIR`, `DEFAULT_LIMIT`, `DEFAULT_TRACKERS` (8 active UDP trackers), `STREAM_BUFFER_MB`, `STREAM_PORT`, `DHT_BOOTSTRAP_NODES` (3 entry points), `PROBE_TIMEOUT` (15s), `PROBE_CANDIDATES` (3), `LLM_MODEL` (sonnet), `LLM_TIMEOUT` (90s, was 30s—cold start headroom), `DOWNLOAD_PROGRESS_INTERVAL` (5s). TPB scoring constants: `TPB_SOURCE_SCORES`, `TPB_STATUS_SCORES`, `TPB_RESOLUTION_LADDER`, `TPB_RESOLUTION_DISTANCE_SCORES`, `TPB_CODEC_SCORES`, `MAX_SEED_SCORE`, `SEED_LOG_SCALE`. Proxy config: `get_proxy()` reads `~/.auto-torrent/config.json` → `proxy_url` field; also checks `AUTO_TORRENT_PROXY` env var. ABB request timeout 45s (via requests-cache session in abb.py).
...
- SMS classifier: `claude -p --json-schema --max-turns 3` required for complex JSON (suggestions array). `--max-turns 2` hits error_max_turns, loses structured_output key
- atb-server Twilio webhook: 15s timeout for TwiML response. LLM calls (Opus ~10-15s) must run in background worker, not sync path, else timeout → retry → duplicate processing
- Deploy-during-download: `git push` triggers service restart → SIGTERM kills background search/download tasks → "Couldn't find" SMS to user
- ABB search through proxy: ~29s per request (slow but stable). Direct server IP blocked by ABB on search queries (homepage 200, search timeout). Residential proxy blocks even homepage (404). Must use proxy + country targeting (`__cr.gb` in config). Fallback on proxy failure → no results
